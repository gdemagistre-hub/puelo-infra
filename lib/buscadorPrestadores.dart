...
import 'buscador_empty.dart';
import 'mensajes/emitir_recibo_sheet.dart';

/// Buscador + empty + post-WhatsApp Doy un pago.
...
class _BuscadorPrestadoresWidgetState extends State<BuscadorPrestadoresWidget> {
  ...
  void _cargarPreferenciaZonaCliente() {
    ...
    _prefProvinciaNombre = (geo['provincia_nombre'] ?? '').toString().trim().isEmpty ? null : geo['provincia_nombre'].toString().trim();
    _prefPartidoNombre = (geo['partido_nombre'] ?? '').toString().trim().isEmpty ? null : geo['partido_nombre'].toString().trim();
    _prefLocalidadNombre = (geo['localidad_nombre'] ?? '').toString().trim().isEmpty ? null : geo['localidad_nombre'].toString().trim();
  ...
  String? get _zonaServerFilter => selectedLocalidadId ?? selectedPartidoId ?? selectedProvinciaId;

  Future<void> _cargarPrestadores({required bool reset}) async {
    if (reset) {
      setState(() { _loading = true; _error = null; _docs = []; _lastDoc = null; _hasMore = true; });
    } else {
    ...
    try {
      ...
      try {
        ...
        if (!reset && _lastDoc != null) fallback = fallback.startAfterDocument(_lastDoc!);
  ...
  Future<QuerySnapshot> _queryPrestadores({required bool reset}) async {
    ...
    if (!reset && _lastDoc != null) query = query.startAfterDocument(_lastDoc!);
    return query.get();
  }
  ...
  Future<void> _abrirWhatsApp(String prestadorUid, Map<String, dynamic> data) async {
    ...
    await PostContactoSheet.show(
      ...
      desdeTarjeta: false,
      onPrimary: () {
        Navigator.push(context, MaterialPageRoute(builder: (_) => TarjetaDigitalWidget(usuarioRef: db.collection('usuarios').doc(prestadorUid))));
      },
      onPagar: () {
        showModalBottomSheet<void>(
          ...
          builder: (_) => EmitirReciboSheet(
            contraparteUidFijo: prestadorUid,
            contraparteNombre: nombreLog.isEmpty ? null : nombreLog,
          ),
        );
      },
    );
  ...
  List<QueryDocumentSnapshot> get _filtrados {
    ...
    var list = _docs.where((doc) {
      ...
      if (!CountryProfile.sameCountry(data, UserSession().countryCode)) {
        return false;
      }
      if (data['list_visible'] == false) return false;
      final profesiones = (data['profesiones'] as List<dynamic>? ?? []).map((e) => e.toString()).toList();
      final cats = (data['categorias_servicio'] as List<dynamic>? ?? []).map((e) => e.toString()).toList();
      final allKeys = [...profesiones, ...cats];
      if (catId != null && !CatalogoOficios.coincide(profesiones: allKeys, categoriaId: catId)) return false;
      if (zonaId != null) {
        final zonaIds = (data['zona_ids'] as List<dynamic>? ?? []).map((e) => e.toString()).toSet();
      ...
      final comercial = (data['nombre_comercial'] ?? data['list_nombre'] ?? '').toString().toLowerCase();
      if (nombre.contains(_searchQuery) || apellido.contains(_searchQuery) || comercial.contains(_searchQuery)) return true;
      return CatalogoOficios.coincide(profesiones: allKeys, texto: _searchQuery);
    }).toList();
    if (_tienePreferenciaZona && zonaId == null) _ordenarPorPreferenciaZona(list);
    return list;
  }
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Padding(
                        ...
                        child: FilterChip(
                          label: Text(rubro, style: TextStyle(fontSize: 13, fontWeight: isSelected ? FontWeight.w600 : FontWeight.w500, color: isSelected ? _clientePrimary : _dark)),
                          selected: isSelected,
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          'Priorizando cerca de ${_prefLocalidadNombre ?? _prefPartidoNombre ?? _prefProvinciaNombre ?? 'tu zona'}',
                          style: TextStyle(fontSize: 12, color: _clientePrimary, fontWeight: FontWeight.w600),
                        ),
                      ),
                    ],
                  ),
                        ...
                        ? Center(
                            child: Column(
                              ...
                              children: [
                                ...
                                OutlinedButton(onPressed: () => _cargarPrestadores(reset: true), child: const Text('Reintentar')),
                              ],
                            ),
                          )
                        : RefreshIndicator(
                            ...
                            child: filtered.isEmpty
                                ? ListView(
                                    ...
                                    children: [const SizedBox(height: 56), _emptyState()],
                                  )
                                : ListView.builder(
                                    ...
                                    itemBuilder: (context, index) {
                                        ...
                                        return Padding(
                                          ...
                                          child: Center(
                                            child: _loadingMore
                                                ...
                                                : TextButton(onPressed: () => _cargarPrestadores(reset: false), child: const Text('Ver más prestadores')),
                                          ),
                                        );
                                      }
                                      final doc = filtered[index];
                                      final data = doc.data() as Map<String, dynamic>;
                                      final promedio = (data['list_promedio'] as num?)?.toDouble() ?? (data['promedioEstrellas'] as num?)?.toDouble() ?? 0.0;
                                      final cantidadEvaluadores = (data['list_n_eval'] as num?)?.toInt() ?? (data['cantidadEvaluadores'] as num?)?.toInt() ?? 0;
                                      final profesiones = data['profesiones'] as List? ?? [];
                                      final badgeRaw = (data['list_badge'] ?? data['badge_prestador'] ?? '').toString().trim();
                                      final badgeLabel = ScoringService.labelBadge(badgeRaw.isEmpty ? null : badgeRaw);
                                      final badgeColors = ScoringService.coloresBadge(badgeRaw.isEmpty ? null : badgeRaw);
                                      ...
                                      return Container(
                                        ...
                                        decoration: BoxDecoration(
                                          ...
                                          border: cerca ? Border.all(color: _clientePrimary.withOpacity(0.25)) : null,
                                        ...
                                        child: Material(
                                          ...
                                          child: InkWell(
                                            borderRadius: BorderRadius.circular(16),
                                            onTap: () {
                                              Navigator.push(context, MaterialPageRoute(builder: (_) => TarjetaDigitalWidget(usuarioRef: doc.reference)));
                                            },
                                            child: Padding(
                                              padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 12),
                                              child: Row(
                                                children: [
                                                  _avatarFila(foto: _fotoUrl(data), initials: _initials(data)),
                                                  const SizedBox(width: 12),
                                                  Expanded(
                                                    child: Column(
                                                      crossAxisAlignment: CrossAxisAlignment.start,
                                                      children: [
                                                        Text(nombreMostrar.isEmpty ? 'Prestador' : nombreMostrar, style: const TextStyle(fontWeight: FontWeight.bold, color: _textColor, fontSize: 15), maxLines: 1, overflow: TextOverflow.ellipsis),
                                                        Text(_labelsOficios(profesiones), style: TextStyle(color: Colors.grey.shade600, fontSize: 13), maxLines: 1, overflow: TextOverflow.ellipsis),
                                                        if (zona.isNotEmpty || cerca)
                                                          Padding(
                                                              ...
                                                              children: [
                                                                if (cerca) ...[
                                                                  Container(
                                                                    ...
                                                                    decoration: BoxDecoration(color: _clientePrimary.withOpacity(0.10), borderRadius: BorderRadius.circular(8)),
                                                                    child: const Text('Cerca', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: _clientePrimary)),
                                                                  ),
                                                                  const SizedBox(width: 6),
                                                                ],
                                                                if (zona.isNotEmpty)
                                                                  Expanded(child: Text(zona, style: TextStyle(color: Colors.grey.shade500, fontSize: 11), maxLines: 1, overflow: TextOverflow.ellipsis)),
                                                              ],
                                                            ),
                                                          ),
                                                      ],
                                                    ...
                                                    children: [
                                                      Row(
                                                        ...
                                                        children: [
                                                          ...
                                                          Text(cantidadEvaluadores > 0 ? promedio.toStringAsFixed(1) : '—', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                                                          Text(' ($cantidadEvaluadores)', style: TextStyle(fontSize: 11, color: Colors.grey.shade500, fontWeight: FontWeight.w600)),
                                                        ],
                                                      ),
                                                      if (badgeLabel.isNotEmpty) ...[
                                                        ...
                                                        Container(
                                                          ...
                                                          decoration: BoxDecoration(color: Color(badgeColors.background), borderRadius: BorderRadius.circular(20)),
                                                          child: Text(badgeLabel, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: Color(badgeColors.foreground))),
                                                        ),
                                                      ],
                                                      ...
                                                      Material(
                                                        ...
                                                        child: InkWell(
                                                          borderRadius: BorderRadius.circular(20),
                                                          onTap: puedeWa ? () => _abrirWhatsApp(doc.id, data) : null,
                                                          child: const Padding(
                                                            ...
                                                            child: Row(
                                                              ...
                                                              children: [
                                                                ...
                                                                SizedBox(width: 4),
                                                                Text('WhatsApp', style: TextStyle(color: Colors.white, fontSize: 11, fontWeight: FontWeight.w700)),
                                                              ],
                                                            ),
                                                          ),
                                                        ),
  ...
  void _limpiarFiltros() {
    ...
    setState(() { _searchQuery = ''; _selectedRubro = 'Todos'; });
    _cargarPrestadores(reset: true);
  }

  Widget _avatarFila({required String? foto, required String initials}) {
    final fallback = Text(initials, style: const TextStyle(color: _clientePrimary, fontWeight: FontWeight.bold, fontSize: 15));
    return CircleAvatar(
      radius: 26,
      backgroundColor: _clientePrimary.withOpacity(0.14),
      child: foto == null
          ? fallback
          : ClipOval(child: Image.network(foto, width: 52, height: 52, fit: BoxFit.cover, errorBuilder: (_, __, ___) => Center(child: fallback))),
    );
  }

  Widget _emptyState() {
    return BuscadorEmptyState(
      rubro: _selectedRubro,
      query: _searchQuery,
      zonaNombre: _prefLocalidadNombre ?? _prefPartidoNombre ?? _prefProvinciaNombre,
      tieneZona: _tienePreferenciaZona,
      onVerTodos: _limpiarFiltros,
      onActualizar: () => _cargarPrestadores(reset: true),
    );
  }
}
