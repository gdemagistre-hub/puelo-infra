...
class MovimientosStore extends ChangeNotifier {
  ...
  Future<void> loadForUser(String? uid) async {
    ...
    _syncing = false;
    notifyListeners();
    // Phase 0.1: denormalizar conteos al doc usuario (scoring)
  ...
  /// Phase 0.1: solo conteos (sin montos) → usuarios/{uid}.stats_negocio
  Future<void> _syncStatsNegocio() async {
    final uid = _uid;
    if (uid == null || uid.isEmpty) return;
    final nMov = _items.length;
    final nCobros = _items.where((m) => m.esCobro).length;
