...
class _TarjetaDigitalWidgetState extends State<TarjetaDigitalWidget> {
  ...
  Future<void> _contactarWhatsApp(String telefono, String nombre, {required String prestadorUid}) async {
    ...
    if (await canLaunchUrl(url)) {
      ...
      _alerta('No se pudo abrir WhatsApp');
    }
  }
  ...
  Future<void> _realizarLlamada(String telefono, {required String prestadorUid, String? prestadorNombre}) async {
    ...
    if (await canLaunchUrl(url)) {
      ...
      _alerta('No se pudo iniciar la llamada');
    }
  }
  ...
  Future<void> _compartirPorWhatsApp(String nombre, String idDocumento) async {
    ...
    if (await canLaunchUrl(url)) {
      ...
      _alerta('No se pudo abrir WhatsApp para compartir');
    }
  }
  ...
  void _onBack() {
    if (Navigator.canPop(context)) {
      ...
      Navigator.pushAndRemoveUntil(
        context,
        MaterialPageRoute(builder: (_) => const HomePageWidget(initialModoPrestador: false)),
        (_) => false,
      );
    }
  }
  ...
  Future<List<_FotoItem>> _cargarFotos(String userId) async {
    ...
    void extraerDeDoc(Map<String, dynamic> d) {
      ...
      for (final img in imgs) {
        ...
        if (img is String) {
          ...
          url = (img['url'] ?? img['downloadURL'] ?? img['src'] ?? '').toString();
          desc = (img['descripcion'] ?? img['caption'] ?? '').toString();
        }
    ...
    try {
      ...
      for (final doc in snap.docs) {
        ...
        if (tipo.isEmpty || tipo == 'portfolio' || tipo == 'trabajo') {
          extraerDeDoc(d);
        }
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        final scoreRaw = datos['list_score_identidad'] ??
            (scoringMap is Map ? scoringMap['score_identidad'] : null);
        ...
        final nRaw = datos['list_n_evaluaciones'] ??
            datos['nEvaluaciones'] ??
            datos['cantidad_evaluaciones'] ??
            datos['cantidadEvaluadores'] ??
            0;
        final cantEval = nRaw is num ? nRaw.toInt() : (int.tryParse('$nRaw') ?? 0);

        final badgeLabel = (badge != null && badge.isNotEmpty)
            ? ScoringService.labelBadge(badge)
            : '';
        final badgeColors = ScoringService.coloresBadge(
          (badge != null && badge.isNotEmpty) ? badge : null,
        );

        String microcopy;
        if (cantEval > 0 && promedio > 0) {
          microcopy = 'Los clientes lo califican con ${promedio.toStringAsFixed(1)}';
        } else if (scoreClamped > 0) {
          microcopy = 'Perfil en Puelo · confianza $scoreClamped/100';
        } else {
          microcopy = 'Perfil en Puelo · todavía sin reseñas de trabajos';
        }
                        ...
                        if (profesiones.isNotEmpty) ...[
                          ...
                          Padding(
                            ...
                            child: Wrap(
                              ...
                              children: profesiones.take(6).map((p) {
                                return Container(
                          ...
                          Padding(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                Flexible(
                                  child: Text(
                                    'Trabaja en ${zonasLista.take(3).join(', ')}',
                                    textAlign: TextAlign.center,
                                    maxLines: 2,
                                            ...
                                            children: [
                                              SizedBox(
                                                ...
                                                child: CircularProgressIndicator(
                                                  value: scoreIdentidad > 0 ? scoreProgress : 0,
                                                  strokeWidth: 5,
