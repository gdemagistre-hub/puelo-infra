...
/// Resultado del parseo OCR del documento
class DatosOcrDocumento {
...
/// Resultado del cotejo perfil vs OCR
...
class DniOcrHelper {
  ...
  bool get disponibleEnPlataforma => !kIsWeb;

  Future<XFile?> capturarFotoDocumento({bool camara = true}) async {
  ...
  Future<String?> leerTextoDeArchivo(String path) async {
    if (kIsWeb) return null;
  ...
  /// Parseo heurístico orientado a DNI AR / documentos latinos
  static DatosOcrDocumento parsearTexto(String texto) {
    ...
    // Número de documento: 7-8 dígitos, con o sin puntos
    String? numero;
    ...
    // Fecha nacimiento dd/mm/yyyy o dd-mm-yyyy
    DateTime? fecha;
    ...
    // Nombres / apellidos por etiquetas del DNI
    ...
    for (var i = 0; i < lineas.length; i++) {
      ...
      if ((l.contains('NOMBRE') || l.contains('GIVEN') || l.contains('NAME')) &&
          !l.contains('APELLIDO') &&
          i + 1 < lineas.length) {
    ...
    // Fallback: líneas en mayúsculas tipo nombre propio
  ...
  /// Reglas de coincidencia pedidas:
  ...
  static ResultadoValidacionDni validarContraPerfil({
    required DatosOcrDocumento ocr,
    ...
    final nombresUser = _tokensPersona(nombreUsuario).map(_norm).toSet();
    final apellidosUser = _tokensPersona(apellidoUsuario).map(_norm).toSet();
  ...
  /// Hash de los datos actuales del perfil (sin timestamp) para invalidar si se editan
  static String hashDatosAncla({
    ...
    final payload = [
      _normDoc(numero),
      _tokensPersona(nombre).map(_norm).join(','),
      _tokensPersona(apellido).map(_norm).join(','),
      _norm(pais ?? ''),
