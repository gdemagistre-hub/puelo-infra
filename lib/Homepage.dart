...
class _HomePageWidgetState extends State<HomePageWidget> {
  ...
  bool get _esInvitado => !UserSession().isLoggedIn;
  ...
  void _selectTab(int index) {
    ...
    // Invitado: solo Home (directorio). El resto es cuenta.
    if (_esInvitado && index != 0) {
      _pedirLogin();
      return;
    }
  ...
  String get _nombreMostrar {
    if (_esInvitado) return 'bienvenido';
    final n = UserSession().nombreCompleto.trim();
    if (n.isEmpty || n.toLowerCase().contains('null')) return 'bienvenido';
        ...
        child: Padding(
          ...
          child: Row(
            children: [
              ...
              GestureDetector(
                onTap: _esInvitado ? _pedirLogin : (_subiendoFoto ? null : _mostrarOpcionesSelfie),
                child: Stack(
                  clipBehavior: Clip.none,
                  children: [
                    _buildAvatarHeader(),
                    ...
                    children: [
                      Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Prestadores cerca tuyo', style: TextStyle(color: Colors.white, fontSize: 17, fontWeight: FontWeight.w800)),
                            SizedBox(height: 6),
                            Text('Ordenados por calificación', style: TextStyle(color: Colors.white70, fontSize: 13)),
                          ],
                        ),
                      ),
                      Icon(Icons.near_me_rounded, color: Colors.white, size: 34),
                    ],
                  ),
                ),
              ),
