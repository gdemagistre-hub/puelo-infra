...
class _HomePageWidgetState extends State<HomePageWidget> {
      ...
      if (esPrestador) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text('Listo: ya podés ofrecer servicios. Completá zona si te falta.'),
    ...
    try {
      ...
      if (vals.isEmpty && consejos.length < 2) {
        consejos.add(_ConsejoItem(
          ...
          onTap: () {
            Navigator.push(context, MaterialPageRoute(builder: (_) => const SolicitarValidacionWidget()));
          },
        ));
      }
      if (consejos.isEmpty) {
  ...
  Future<void> _ejecutarBatchScoring() async {
    ...
    try {
      final r = await ScoringService.ejecutarBatchDiario(
        onProgress: (hechos, _) => debugPrint('Scoring batch: $hechos usuarios'),
      );
      if (!mounted) return;
      Navigator.pop(context);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Badges actualizados: ${r.actualizados} de ${r.procesados} usuarios'),
      ...
      if (!mounted) return;
      Navigator.pop(context);
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error al recalcular: $e')));
    } finally {
    ...
    try {
      ...
      if (doc.exists) {
        ...
        if (profesiones.isEmpty || localidades.isEmpty || !esTrabajador) {
          if (context.mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Para que te contacten, primero cargá tus oficios y zona de trabajo.')),
            );
            Navigator.push(context, MaterialPageRoute(builder: (_) => const RegistroTrabajadorWidget()))
              ...
              child: Row(
                ...
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.location_on_outlined, color: _clientePrimary, size: 24),
                  ),
                  ...
                  const Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('¿Dónde necesitás el servicio?',
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
                    ...
                    child: Row(
                      children: [
                        Container(
                          ...
                          child: const Icon(Icons.handyman_outlined, color: Colors.white, size: 26),
                        ),
                        ...
                        const Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text('¿Ofrecés un oficio?',
                                  style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
                              SizedBox(height: 4),
                              Text('Empezá a recibir clientes en 3 pasos',
          ...
          const Padding(
            padding: EdgeInsets.symmetric(horizontal: 16),
            child: Text('Según ranking diario de demanda',
            ...
            Padding(
              ...
              child: GridView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return _buildCategoryIcon(s.icon, s.label, color, () => _irABuscador(s.label));
                },
              ),
            ),
          ...
          const Padding(
            padding: EdgeInsets.symmetric(horizontal: 16),
            child: Text(AppCopy.seccionActividad, style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
          ),
          const SizedBox(height: 12),
          Container(
            ...
            child: Row(
              children: [
                ...
                const SizedBox(width: 12),
                const Expanded(
                  child: Text(AppCopy.actividadVacia,
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
                    ...
                    children: [
                      ...
                      const Expanded(
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            Text('Tu tarjeta para que te contacten',
                                style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 16)),
                            SizedBox(height: 4),
                            Text('Pasale tu ficha al cliente por WhatsApp',
                                style: TextStyle(color: Colors.white, fontSize: 13, height: 1.3)),
                          ],
                        ),
                      ),
          ...
          const Padding(
            padding: EdgeInsets.symmetric(horizontal: 16),
            child: Text('Tu trabajo', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
          ),
          const SizedBox(height: 12),
          Padding(
            padding: const EdgeInsets.symmetric(horizontal: 12),
          ...
          const Padding(
            padding: EdgeInsets.symmetric(horizontal: 16),
            child: Text('Priorizamos lo que te hace aparecer en búsquedas',
                style: TextStyle(fontSize: 12, color: Colors.grey)),
          ),
          const SizedBox(height: 12),
          if (_cargandoConsejos)
            ...
            const Padding(padding: EdgeInsets.all(16), child: Text('No hay pendientes por ahora.'))
          else
          ...
          if (AppEnv.showDevTools) ...[
            ...
            Padding(
              ...
              child: OutlinedButton.icon(
                ...
                label: Text(_corriendoBatch ? 'Recalculando…' : 'Recalcular badges (solo dev)'),
                style: OutlinedButton.styleFrom(
            ...
            const Padding(
              ...
              child: Text('Solo visible en debug/staging. No aparece en producción.',
  ...
  Widget _buildVisibilidadCard() {
    if (_visibilidadCargando) {
      return const Padding(
        ...
        child: Center(child: SizedBox(height: 36, width: 36, child: CircularProgressIndicator(strokeWidth: 2))),
      );
    }
    final Color bg = _estaVisible ? const Color(0xFFECFDF5) : const Color(0xFFFFF7ED);
    final Color border = _estaVisible ? const Color(0xFF6EE7B7) : const Color(0xFFFDBA74);
    final Color iconColor = _estaVisible ? const Color(0xFF059669) : const Color(0xFFEA580C);
    final String titulo = _estaVisible ? 'Estás visible para clientes' : 'Todavía no aparecés en búsquedas';
    ...
    return Container(
      ...
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Icon(_estaVisible ? Icons.visibility_outlined : Icons.visibility_off_outlined, color: iconColor, size: 28),
          const SizedBox(width: 12),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(titulo, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 15, color: iconColor)),
                const SizedBox(height: 4),
                Text(cuerpo, style: const TextStyle(fontSize: 13, color: Color(0xFF334155), height: 1.35)),
                const SizedBox(height: 10),
                ClipRRect(
                  borderRadius: BorderRadius.circular(4),
                  child: LinearProgressIndicator(
                    value: _pasosTotales == 0 ? 0 : _pasosCompletos / _pasosTotales,
                    minHeight: 6,
                ...
                const SizedBox(height: 4),
                Text('$_pasosCompletos de $_pasosTotales pasos para que te contacten',
  ...
  Widget _buildCategoryIcon(IconData icon, String label, Color accent, VoidCallback onTap) {
    return InkWell(
      onTap: onTap,
      borderRadius: BorderRadius.circular(16),
      child: Column(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            const SizedBox(width: 12),
            Expanded(
              child: Text(label, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13, height: 1.25)),
            ),
          ],
        ),
      ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 2),
                  Text(c.body, style: TextStyle(color: Colors.grey.shade600, fontSize: 12)),
                ],
              ),
            ),
