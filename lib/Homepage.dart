...
class _HomePageWidgetState extends State<HomePageWidget> {
  ...
  Future<void> _cargarEstadoVisibilidadYConsejos() async {
    ...
    if (uid == null) {
      if (!mounted) return;
      setState(() { _consejos = []; _cargandoConsejos = false; _visibilidadCargando = false; });
      return;
    }
    try {
      ...
      if (mounted) setState(() { _cargandoConsejos = false; _visibilidadCargando = false; });
    }
  }
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                ListTile(
                  leading: Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: primaryColor.withOpacity(0.12), borderRadius: BorderRadius.circular(12)), child: Icon(Icons.photo_camera_outlined, color: primaryColor)),
                  title: const Text('Tomar selfie'),
                  subtitle: const Text('Usá la cámara frontal'),
                  onTap: () { Navigator.pop(ctx); _elegirYSubirFoto(ImageSource.camera); },
                ),
                ListTile(
                  leading: Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: primaryColor.withOpacity(0.12), borderRadius: BorderRadius.circular(12)), child: Icon(Icons.photo_library_outlined, color: primaryColor)),
                  title: const Text('Elegir de galería'),
                  onTap: () { Navigator.pop(ctx); _elegirYSubirFoto(ImageSource.gallery); },
                ),
                if (_urlFotoPerfil != null && _urlFotoPerfil!.isNotEmpty)
                  ListTile(
                    leading: Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.red.withOpacity(0.1), borderRadius: BorderRadius.circular(12)), child: const Icon(Icons.delete_outline, color: Colors.red)),
  ...
  Future<void> _quitarFotoPerfil() async {
    ...
    try {
      ...
      final session = UserSession();
      if (session.datosCompletos != null) session.datosCompletos!.remove('url_foto_perfil');
              ...
              child: Stack(clipBehavior: Clip.none, children: [
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    image: (_urlFotoPerfil != null && _urlFotoPerfil!.isNotEmpty) ? DecorationImage(image: NetworkImage(_urlFotoPerfil!), fit: BoxFit.cover) : null,
                  ),
                  alignment: Alignment.center,
                  child: (_urlFotoPerfil == null || _urlFotoPerfil!.isEmpty)
                      ? Text(_getInitials(), style: TextStyle(color: primaryColor, fontWeight: FontWeight.w800, fontSize: 15))
                      : null,
                ),
                Positioned(
                  right: -2, bottom: -2,
    ...
    return CustomScrollView(slivers: [
      ...
      SliverToBoxAdapter(child: Transform.translate(
        ...
        child: Padding(
          ...
          child: Material(
            color: Colors.white, elevation: 6, shadowColor: _clientePrimary.withOpacity(0.25), borderRadius: BorderRadius.circular(18),
            child: InkWell(
              borderRadius: BorderRadius.circular(18), onTap: () => _abrirBuscador(),
              child: Padding(
                padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 16),
                child: Row(children: [
                  Container(width: 44, height: 44, decoration: BoxDecoration(color: _clientePrimary.withOpacity(0.12), borderRadius: BorderRadius.circular(12)), child: const Icon(Icons.search_rounded, color: _clientePrimary, size: 24)),
      ...
      SliverPadding(
        padding: const EdgeInsets.symmetric(horizontal: 16),
        sliver: SliverGrid(
          gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, crossAxisSpacing: 12, mainAxisSpacing: 12, childAspectRatio: 1.18),
          delegate: SliverChildBuilderDelegate((context, index) {
            final cat = _categorias[index];
            final Color accent = (cat['color'] as Color?) ?? _clientePrimary;
            return Material(
              color: Colors.white, borderRadius: BorderRadius.circular(16), elevation: 1.5, shadowColor: Colors.black.withOpacity(0.08),
              child: InkWell(
                borderRadius: BorderRadius.circular(16),
                onTap: () => _abrirBuscador(oficio: cat['id'] as String),
                splashColor: accent.withOpacity(0.12), highlightColor: accent.withOpacity(0.05),
                child: Padding(
                  padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 16),
                  child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Container(
                      width: 52, height: 52,
                      decoration: BoxDecoration(color: accent.withOpacity(0.12), borderRadius: BorderRadius.circular(16)),
                      child: Icon(cat['icon'] as IconData, color: accent, size: 28),
                    ),
                    const SizedBox(height: 12),
                    Text(cat['label'] as String, textAlign: TextAlign.center, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Color(0xFF1E293B), height: 1.15), maxLines: 1, overflow: TextOverflow.ellipsis),
                  ]),
                ),
              ),
            );
