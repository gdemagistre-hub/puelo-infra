...
class ScoringService {
  ...
  static LayerScoreResult calcularScoreIdentidad(
    ...
    if (esTrabajador) {
      ...
      // Capacitaciones / cursos (máx 3 pts) — evidencia de profesionalización
      final caps = data['capacitaciones'] as List<dynamic>? ?? [];
      final nCaps = caps.length.clamp(0, 3);
      detalle['capacitaciones'] = nCaps;
    }
  ...
  /// Phase 0: priorizado por impacto estimado y tope de 6 tips.
  static List<Map<String, String>> generarConsejosConfianza(
    Map<String, dynamic> data, {
    int fotosPortfolio = 0,
  }) {
    final candidates = <({int prio, String id, String title, String body})>[];

    void tip(int prio, String id, String title, String body) {
      candidates.add((prio: prio, id: id, title: title, body: body));
    }

    // Impacto alto primero (OCR > foto perfil > validación social > resto)
    ...
    final esTrabajador =
        data['es_trabajador'] == true || data['rol'] == 'trabajador';
    if (esTrabajador && profesiones.isEmpty) {
      tip(
        55,
        'oficios',
    ...
    if (esTrabajador && fotosPortfolio < 1) {
      tip(
        48,
        'fotos_trabajo',
    ...
    final caps = data['capacitaciones'] as List<dynamic>? ?? [];
    if (esTrabajador && caps.isEmpty) {
      tip(
        42,
        'capacitaciones',
        'Cargá un curso o capacitación',
        'Muestra que te formás. No afecta el score de crédito, sí tu profesionalismo.',
      );
    }
    ...
    if (esTrabajador && nEval == 0 && cant == 0) {
      tip(
        40,
        'evals',
    ...
    // Actividad de negocio (Mis Números) — soft tip, no bloquea
    final statsNeg = data['stats_negocio'];
    final tieneMovs = statsNeg is Map &&
        ((statsNeg['n_movimientos'] as num?)?.toInt() ?? 0) > 0;
    if (esTrabajador && !tieneMovs) {
      tip(
        35,
        'mis_numeros',
        'Registrá tus cobros en Mis Números',
        'Ayuda a mostrar que tu trabajo genera actividad real (solo vos lo ves).',
      );
    }
  ...
  // ---------------------------------------------------------------------------
  // CAPA E — COMPORTAMIENTO (Phase 0, señales de actividad / negocio real)
  // ---------------------------------------------------------------------------

  /// Señales de que el prestador tiene actividad real (sin montos sensibles).
  /// Usa stats_negocio denormalizado (si existe) + signals del doc.
  /// NO se muestra como score principal; alimenta credito preview y ranking.
  static LayerScoreResult calcularScoreComportamiento(
    Map<String, dynamic> data, {
    required int fotosPortfolio,
    required int fotosClientes,
    required int nEvalTrabajo,
    required int nValidaciones6m,
  }) {
    final detalle = <String, int>{};
    int raw = 0;

    void addPts(String key, int pts) {
      if (pts <= 0) return;
      detalle[key] = pts;
      raw += pts;
    }
    ...
    final esTrabajador =
        data['es_trabajador'] == true || data['rol'] == 'trabajador';
    if (!esTrabajador) {
      return LayerScoreResult(raw: 0, score: 0, detalle: detalle);
    }

    // Portfolio + evidencia visual
    addPts('fotos_portfolio', fotosPortfolio.clamp(0, 4));
    addPts('fotos_clientes', fotosClientes.clamp(0, 3));

    // Trayectoria de evaluaciones / validaciones
    if (nEvalTrabajo >= 5) {
      addPts('evals_5plus', 4);
    } else if (nEvalTrabajo >= 2) {
      addPts('evals_2plus', 2);
    } else if (nEvalTrabajo >= 1) {
      addPts('evals_1', 1);
    }
    if (nValidaciones6m >= 3) {
      addPts('vals_3plus', 2);
    } else if (nValidaciones6m >= 1) {
      addPts('vals_1', 1);
    }

    // Capacitaciones
    final caps = data['capacitaciones'] as List<dynamic>? ?? [];
    addPts('capacitaciones', caps.length.clamp(0, 2));

    // stats_negocio denormalizado (escrito por Mis Números stores)
    final stats = data['stats_negocio'];
    if (stats is Map) {
      final nMov = (stats['n_movimientos'] as num?)?.toInt() ?? 0;
      final nCobros = (stats['n_cobros'] as num?)?.toInt() ?? 0;
      final nFiadosCobrados =
          (stats['n_fiados_cobrados'] as num?)?.toInt() ?? 0;
      final tieneFondo = stats['tiene_fondo_emergencia'] == true;

      if (nMov >= 10) {
        addPts('movs_10plus', 3);
      } else if (nMov >= 3) {
        addPts('movs_3plus', 2);
      } else if (nMov >= 1) {
        addPts('movs_1', 1);
      ...
      if (nCobros >= 5) {
        addPts('cobros_5plus', 2);
      } else if (nCobros >= 1) {
        addPts('cobros_1', 1);
      }
      if (nFiadosCobrados >= 1) {
        addPts('fiados_cobrados', 1);
      }
      if (tieneFondo) {
        addPts('fondo_emergencia', 1);
      }
    }

    // Antigüedad mínima como señal de estabilidad (no gaming)
    final dias = _diasDesdeAlta(data) ?? 0;
    if (dias >= 90) {
      addPts('antiguedad_90d', 2);
    } else if (dias >= 30) {
      addPts('antiguedad_30d', 1);
    }

    final normalizado = _normalizar(raw, techoComportamiento);
    return LayerScoreResult(
      raw: raw,
      score: normalizado,
      detalle: detalle,
  ...
  /// Phase 0: incorpora score_comportamiento (15%) y deja 25% reservado para ML futuro.
  static int calcularScoreCreditoPreview({
    required int scoreIdentidad,
    required int scoreServicio,
    required int scoreCliente,
    int scoreComportamiento = 0,
    bool esTrabajador = false,
  }) {
    // Pesos Phase 0: identidad 35%, reputación 25%, comportamiento 15%, reservado 25%
    final reputacion = esTrabajador ? scoreServicio : scoreCliente;
    final v = (0.35 * scoreIdentidad) +
        (0.25 * reputacion) +
        (0.15 * scoreComportamiento) +
        (0.25 * 0);
    ...
    try {
      ...
      for (final entry in allUserDocs.entries) {
        ...
        try {
          ...
          final comportamiento = calcularScoreComportamiento(
            data,
            fotosPortfolio: fp,
            fotosClientes: fc,
          ...
          final creditoPreview = calcularScoreCreditoPreview(
            ...
            scoreComportamiento: comportamiento.score,
          ...
          batch.set(
            ref,
            {
              ...
              'scoring': {
                ...
                'score_comportamiento': comportamiento.score,
                'score_comportamiento_raw': comportamiento.raw,
                'score_comportamiento_detalle': comportamiento.detalle,
              ...
              // Campos livianos para ranking / Feature Store futuro
              'list_score_identidad': identidad.score,
  ...
  /// Actualiza conteos no sensibles en usuarios/{uid}.stats_negocio.
  /// Llamar desde MovimientosStore / FiadosStore / MetasStore al guardar.
  ...
  static Future<void> actualizarStatsNegocio({
    ...
    try {
      ...
      // Best-effort: no romper el flujo de Mis Números
      // ignore: avoid_print
